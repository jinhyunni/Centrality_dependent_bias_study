void plotMake_ratio_dNdpT_dirMB_MCcompare()
{
	TFile *input1 = new TFile("pAu200GeV_option3_ratio_dNdpT_MCcompare.root", "read");
  
    //dndeta in MB events
    
    TH1D *ver2npl_8303_externalOn = (TH1D*)input1 -> Get("dir_pTratio_ver2npl_8303_externalOn");
    TH1D *ver3kiaf_8307_externalOff = (TH1D*)input1 -> Get("dir_pTratio_ver3kiaf_8307_externalOff");
    TH1D *ver3npl_8303_externalOff = (TH1D*)input1 -> Get("dir_pTratio_ver3npl_8303_externalOff");
    TH1D *ver3kiaf_8303_externalOff = (TH1D*)input1 -> Get("dir_pTratio_ver3kiaf_8303_externalOff");
	...
	//	TH1D *htmp = (TH1D*)gPad -> DrawFrame(-1.5, 2.6, 1.5, 5);
        ...
        //full-rapidity
		TH1D *htmp = (TH1D*)gPad -> DrawFrame(0, 0.8, 10, 2.0);

		htmp -> GetXaxis() -> SetTitle("p_{T}");
		htmp -> GetYaxis() -> SetTitle("(#frac{1}{N_{event}} #frac{dN^{#gamma^{dir}}}{dp_{T}})_{target MC} / (#frac{1}{N_{event}} #frac{dN^{#gamma^{dir}}}{dp_{T}})_{reference MC}");
