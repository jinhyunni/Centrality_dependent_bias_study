void plotMake_ratio_dNdeta_dirMB_MCcompare()
{
	TFile *input1 = new TFile("pAu200GeV_option3_ratio_dNdeta_MCcompare.root", "read");
  
    //dndeta in MB events
    
    TH1D *ver2npl_8303_externalOn = (TH1D*)input1 -> Get("dir_ratio_ver2npl_8303_externalOn");
    TH1D *ver3kiaf_8307_externalOff = (TH1D*)input1 -> Get("dir_ratio_ver3kiaf_8307_externalOff");
    TH1D *ver3npl_8303_externalOff = (TH1D*)input1 -> Get("dir_ratio_ver3npl_8303_externalOff");
    TH1D *ver3kiaf_8303_externalOff = (TH1D*)input1 -> Get("dir_ratio_ver3kiaf_8303_externalOff");
	...
	//	TH1D *htmp = (TH1D*)gPad -> DrawFrame(-1.5, 2.6, 1.5, 5);
        ...
        //full-rapidity
		TH1D *htmp = (TH1D*)gPad -> DrawFrame(-5, 0.8, 5, 1.8);

		htmp -> GetXaxis() -> SetTitle("#eta");
		htmp -> GetYaxis() -> SetTitle("(#frac{1}{N_{event}} #frac{dN^{#gamma^{dir}}}{d#eta})_{target MC} / (#frac{1}{N_{event}} #frac{dN^{#gamma^{dir}}}{d#eta})_{reference MC}");
