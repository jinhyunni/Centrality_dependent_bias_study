void plotMake_dirInEx_protondNdeta()
{
	...
	TH1D *new_pionEta = (TH1D*)input1 -> Get("dirAdded_protonEta");
	TH1D *old_pionEta = (TH1D*)input2 -> Get("dirEx_protonEta");

	TProfile *eventNnew = (TProfile*)input3 -> Get("new_eventN_00000");
	TProfile *eventNold = (TProfile*)input3 -> Get("old_eventN");


	new_pionEta -> Scale(1./(eventNnew -> GetBinContent(2212) * new_pionEta -> GetBinWidth(1)));
	old_pionEta -> Scale(1./(eventNold -> GetBinContent(2212) * old_pionEta -> GetBinWidth(1)));
	...
	TCanvas *c1 = new TCanvas("", "", 800, 600);
	{
		...
		htmp -> GetYaxis() -> SetTitle("1/N_{evt} dN^{p^{#pm}}/d#eta");
