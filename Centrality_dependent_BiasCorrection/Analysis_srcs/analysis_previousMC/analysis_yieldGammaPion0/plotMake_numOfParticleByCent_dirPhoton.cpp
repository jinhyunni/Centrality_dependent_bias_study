void plotMake_numOfParticleByCent_dirPhoton()
{
	TFile *input1 = new TFile("pAu200GeV_option3_dirAdded_decayOn_TH2DdirCent.root", "read");
	...
	TCanvas *c1 = new TCanvas("", "", 800, 600);
	{
		...
		TH1D *htmp = (TH1D*)gPad -> DrawFrame(0, 0, 100, 0.5e4);

		htmp -> GetXaxis() -> SetTitle("centrality(%)");
		htmp -> GetYaxis() -> SetTitle("# of #gamma^{dir}");
    
        num_lowR -> SetMarkerStyle(24);
        num_lowR -> SetMarkerColor(4);
        num_lowR -> SetLineColor(4);
        num_lowR -> Draw("p same");
