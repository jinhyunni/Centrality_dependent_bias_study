void analyis_NcollByCent_inMidJetEvents()
{
    ...
    for(int i=0; i<5; i++)
    {
        TString outputname1 = Form("ncoll_midpion0_%dGeV_cent1", 2*i+3);
        TString outputname2 = Form("ncoll_midpion0_%dGeV_cent2", 2*i+3);
        TString outputname3 = Form("ncoll_midpion0_%dGeV_cent3", 2*i+3);
        TString outputname4 = Form("ncoll_midpion0_%dGeV_cent4", 2*i+3);
        TString outputname5 = Form("ncoll_midpion0_%dGeV_cent5", 2*i+3);
    ...
    //output
    //------
    TFile *output = new TFile("pAu200GeV_p8303ver2_option3_NcollByCent_inMidJetEvents.root", "recreate");
    for(int i=0; i<5; i++)
    {       
        for(int j=0; j<5; j++)
        {
            ncoll_midpion0_cent[i][j] -> Write();
        }
    }
    output -> Close();
