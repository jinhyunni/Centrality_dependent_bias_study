...
void analysis_NcollTagging_pion0_pTeta_byNcoll_byCent()
{
	...
	{
		charge_pTetaCent[ncoll] = (TH3D*)input -> Get(Form("pTetaCent_pion0_ncollClass%d", ncoll+1));
	}
	...
	for(int ncoll=0; ncoll<6; ncoll++)
	{
		for(int cent=0; cent<5; cent++)
		{
			...
			charge_pTeta[ncoll][cent] -> SetName(Form("pTeta_pion0_ncollClass%d_centClass%d", ncoll+1, cent+1));
		}
	}
