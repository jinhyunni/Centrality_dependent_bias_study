...
void plotMake_correlations_maxpT_vs_avgChargepT_corrWithMidPion0()
{
    TFile *input = new TFile("../pre_processed/pAu200GeV_p8303_ver5_option3_correlations_withPion0_TProfile_avgpTSumCh_vs_maxpT.root", "read");	
	TProfile *maxpT_vs_avgpT = (TProfile*)input -> Get("maxpT_vs_avgChpT_corrWithMidPion0");
    ...
    TCanvas *c1 = new TCanvas("", "", 800, 600);
    {
        ...
        htmp -> GetXaxis() -> SetTitle("Max p_{T} of #pi^{0} at |#eta|<1 (GeV/c)");
        htmp -> GetYaxis() -> SetTitle("#LT #sump^{ch}_{T} #GT at -3.9 < #eta < -3.0 (GeV/c)");

        maxpT_vs_avgpT_rebinned -> SetMarkerStyle(28);
        maxpT_vs_avgpT_rebinned -> SetMarkerColor(kPink);
        maxpT_vs_avgpT_rebinned -> SetLineColor(kPink);
