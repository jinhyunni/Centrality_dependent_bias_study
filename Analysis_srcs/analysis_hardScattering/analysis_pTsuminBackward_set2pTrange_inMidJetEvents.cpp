...
{   
    //input
    //-----
    TFile *input = new TFile("../pre_processed/pAu200GeV_p8303ver4_option3_TH1DpTsumTriggerpT_set2pTrange_maxpTcut_inMidJetEvents.root", "read");

    TH1D *pTsum[7];
    pTsum[0] = (TH1D*)input -> Get("pTsumTriggerpT_maxpT2to3GeV");
    pTsum[1] = (TH1D*)input -> Get("pTsumTriggerpT_maxpT3to5GeV");
    pTsum[2] = (TH1D*)input -> Get("pTsumTriggerpT_maxpT5to7GeV");
    pTsum[3] = (TH1D*)input -> Get("pTsumTriggerpT_maxpT7to9GeV");
    pTsum[4] = (TH1D*)input -> Get("pTsumTriggerpT_morethan9GeV");
    pTsum[5] = (TH1D*)input -> Get("pTsumTriggerpT_maxpT11to15GeV");
    pTsum[6] = (TH1D*)input -> Get("pTsumTriggerpT_maxpT15to20GeV");
    ...
    //Output
    //------
    TFile *output = new TFile("pAu200GeV_p8303ver4_option3_pTsuminBackward_set2pTrange_maxpTcut_inMidJetEvents.root", "recreate");
    for(int i=0; i<7; i++)
    {
